...
OBJ_ASSET_FILE = "cylinder_0.urdf"
# ===============================================================
...
obj_pose.p = gymapi.Vec3(0.0, 0.0, 0.6)    # 桌面上方一点点，避免初始穿插；按你的桌面实际高度调整
...
# ---- 将手的 DOF 设定为给定关节角 ----
q_over = {
    "joint_0": 0.011, "joint_1": 1.216, "joint_2": 0.082,  "joint_3": 0.615,
    "joint_4": 0.029, "joint_5": 1.018, "joint_6": 0.073,  "joint_7": 0.696,
    "joint_8": 0.149, "joint_9": 1.095, "joint_10": 0.835, "joint_11": 0.230,
    "joint_12": 1.339, "joint_13": 0.467, "joint_14": 0.915, "joint_15": -0.263,
}

# 1) 名称->索引
hand_dof_count = gym.get_asset_dof_count(hand_asset)
name_to_idx = {}
for i in range(hand_dof_count):
    nm = gym.get_asset_dof_name(hand_asset, i)
    name_to_idx[nm] = i

# 2) 取 actor 级 DOF 属性并切到位置控制
dprops = gym.get_actor_dof_properties(env, hand_actor)
dprops['driveMode'][:] = gymapi.DOF_MODE_POS
dprops['stiffness'][:] = 40.0   # 视手模型需要可调
dprops['damping'][:] = 2.0
gym.set_actor_dof_properties(env, hand_actor, dprops)

# 3) 组装目标关节角
import numpy as np
qpos = np.zeros(hand_dof_count, dtype=np.float32)
for nm, val in q_over.items():
    if nm not in name_to_idx:
        raise RuntimeError(f"关节名不存在于URDF: {nm}")
    qpos[name_to_idx[nm]] = val

# 4) 设定初始状态（pos, vel=0），并下发位置目标
dof_states = np.zeros(hand_dof_count, dtype=gymapi.DofState.dtype)
dof_states['pos'] = qpos
dof_states['vel'] = 0.0
gym.set_actor_dof_states(env, hand_actor, dof_states, gymapi.STATE_ALL)
gym.set_actor_dof_position_targets(env, hand_actor, qpos)
